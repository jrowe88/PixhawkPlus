...
void LedControl::Process()
{
	...
	{						
		...
		//GPS Fix		
		SetGPSLights(_messages->FixType);
...
{
	for (size_t i = 0; i < _ledDriver.numPixels(); i++)
	{
...
void LedControl::SetGPSLights(uint8_t gpsStatus)
{
	...
	case NoFix:
		...
		break;
	case Fix2D:
		SetPixelsColorPattern(0, _rainbow);
		SetPixelsColorPattern(1, _rainbow, 8);
		SetPixelsColorPattern(2, _rainbow, 16);
		SetPixelsColorPattern(3, _rainbow, 32);
		SetPixelsColorPattern(4, _rainbow, 40);
		SetPixelsColorPattern(5, _rainbow, 48);   
		SetPixelsColorPattern(6, _rainbow, 56);
		break;
	case Fix3D:
